...
// function to check if the password is valid, returns true if it is
// also updates the requirements list shown to the user
function isValidPass(password, reg_req) {
    password.value = password.value.trim();

    if (!/\d/.test(password.value)) {
        // check if password contains a number
        error_msg.textContent = "Password must contain a number."; // set error message
        error_msg.classList.add("show"); // sets the error to display
        return false; // password is invalid
    } else if (!/[a-z]/.test(password.value)) {
        // check if password contains a lowercase letter
        error_msg.textContent = "Password must contain a lowercase letter."; // set error message
        error_msg.classList.add("show"); // sets the error to display
        return false; // password is invalid
    } else if (!/[A-Z]/.test(password.value)) {
        // check if password contains an uppercase letter
        error_msg.textContent = "Password must contain an uppercase letter."; // set error message
        error_msg.classList.add("show"); // sets the error to display
        return false; // password is invalid
    } else if (!/[!@#$%\^&*()\-_+={}\[\]|\\:;"'<>,.\/?~]/.test(password.value)) {
        // check if password contains special characters (!@#$%^&*()-_+={}[]|\;:"'<>,./?~)
        error_msg.textContent = "Password must contain a special character."; // set error message
        error_msg.classList.add("show"); // sets the error to display
        return false; // password is invalid
    } else if (password.value.length < 8) {
        // check if password is contains 8+ characters
        error_msg.textContent ="Password must contain at least 8 characters."; // set error message
        error_msg.classList.add("show"); // sets the error to display
        return false; // password is invalid
    } else {
...
function isValidConfirmPass(password, confirm_password, error_msg) {
