...
wiggle_amount = config.getint("Settings", "wiggle_amount")
...
def wiggle_bubble(location: list, size: int, amount: int) -> list:
    if (location[0] - amount) >= size and (location[0] + amount) <= (WIDTH - size):
        location[0] = rng(location[0]-amount, location[0]+amount)
    elif (location[0] - amount) < size:
        location[0] = rng(size, location[0]+amount)
    elif (location[0] + amount) > (WIDTH - size):
        location[0] = rng(location[0]-amount, (WIDTH - size))
            
    if (location[1] - amount) >= size and (location[1] + amount) <= (HEIGHT - size):
        location[1] = rng(location[1]-amount, location[1]+amount)
    elif (location[1] - amount) < size:
        location[1] = rng(size, location[1]+amount)
    elif (location[1] + amount) > (HEIGHT - size):
        location[1] = rng(location[1]-amount, (HEIGHT - size))
...
async def main_game():
    ...
    while playing:
        ...
        if wiggle_amount != 0:
            rand_x, rand_y = wiggle_bubble([rand_x, rand_y], rand_size, wiggle_amount)
...
def set_wiggle_amount(value):
    global wiggle_amount
    wiggle_amount =  int(value)
    config.set("Settings", "wiggle_amount", str(wiggle_amount))
...
menu = pygame_menu.Menu("Settings", WIDTH, HEIGHT, theme=pygame_menu.themes.THEME_BLUE)
...
menu.add.text_input("Wiggle amount: ", default=wiggle_amount, onchange=set_wiggle_amount, font_size=25)
